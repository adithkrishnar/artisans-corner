export default function StarRating({ rating, size = 'sm', interactive = false, value = 0, onChange = null }) {
  const sizes = { xs: 12, sm: 14, md: 18, lg: 24 };
  const px = sizes[size] || 14;
  const display = interactive ? value : rating;

  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map(star => (
        <button key={star} type="button"
          onClick={interactive && onChange ? () => onChange(star) : undefined}
          className={interactive ? 'transition-transform hover:scale-125' : 'cursor-default'}
          style={{ background: 'none', border: 'none', padding: '1px', cursor: interactive ? 'pointer' : 'default' }}>
          <svg width={px} height={px} viewBox="0 0 20 20" fill={star <= Math.round(display) ? '#C47A4A' : '#E8DED5'}>
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
        </button>
      ))}
    </div>
  );
}